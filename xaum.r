...
tmp=dbGetQuery(con,'select * from xaum where Date>"2001-04-01" and Date<"2012-03-01 order by Date"')
dbDisconnect(con)
XAUM=xts(tmp[,-1],as.Date(tmp[,1],"%Y-%m-%d"))
...
chartSeries(XAUM,TA=c(addVo(),addEMA(n=13,col="red"),addSAR(),addMYADX(m=5),addMACD(fast=5,slow=7,signal=5),addBU(m=13),addBE(m=13),addRSI(n=13)))
